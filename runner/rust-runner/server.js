...
async function handleRunRust(req, res) {
    let bodyText = ''
    try {
    ...
    try {
        workspace = await fs.mkdtemp(path.join(os.tmpdir(), 'algo-rust-runner-'))
        await fs.writeFile(path.join(workspace, 'main.rs'), code, 'utf8')

        const compileRaw = await runCommand('rustc', ['--edition=2021', '-O', '-o', 'main', 'main.rs'], {
            cwd: workspace,
...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && url.pathname === '/run/rust') {
        handleRunRust(req, res).catch(error => {
            const message = error instanceof Error ? error.message : 'unknown error'
            sendJson(res, 500, {
                error: 'runner_internal_error',
                message: 'Rust 运行服务内部错误。',
...
server.listen(PORT, HOST, () => {
    ...
    console.log(`rust-runner listening on ${HOST}:${PORT}`)
})
