...
async function handleRunCpp(req, res) {
    let bodyText = ''
    try {
    ...
    try {
        workspace = await fs.mkdtemp(path.join(os.tmpdir(), 'algo-cpp-runner-'))
        await fs.writeFile(path.join(workspace, 'main.cpp'), code, 'utf8')

        const compileRaw = await runCommand(
            'g++',
            ['-std=c++17', '-O2', '-pipe', '-o', 'main', 'main.cpp'],
            {
                cwd: workspace,
...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && url.pathname === '/run/cpp') {
        handleRunCpp(req, res).catch(error => {
            const message = error instanceof Error ? error.message : 'unknown error'
            sendJson(res, 500, {
                error: 'runner_internal_error',
                message: 'C++ 运行服务内部错误。',
...
server.listen(PORT, HOST, () => {
    ...
    console.log(`cpp-runner listening on ${HOST}:${PORT}`)
})
